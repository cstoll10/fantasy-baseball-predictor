...
function fmt(val, cat) {
  if (val == null) return "—";
  if (["OBP","AVG","SLG","ERA","WHIP"].includes(cat)) return Number(val).toFixed(3);
  return Math.round(val * 10) / 10;
}

function scaleTo600(consensus, cat, pa) {
  if (consensus == null || !pa) return null;
  const counting = ["R","HR","RBI","SB","H","TB","BB","SO"];
  if (!counting.includes(cat)) return consensus;
  return Math.round((consensus / pa) * 600 * 10) / 10;
}
...
function SystemTable({ player, per600, cats }) {
  ...
  if (!available.length) return <div style={{color:"#444",fontSize:12}}>No system data available.</div>;
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,fontFamily:"'DM Mono',monospace"}}>
        <thead>
          <tr>
            ...
            {cats.map(c=>(
              <th key={c} style={{textAlign:"right",padding:"4px 5px",color:"#444",fontWeight:600,fontSize:10}}>{c}</th>
            ))}
          </tr>
        </thead>
        <tbody>
              ...
              <tr key={sys} style={{borderTop:"1px solid #1a1a2e"}}>
                <td style={{padding:"5px 8px"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5}}>
                    <span style={{width:6,height:6,borderRadius:"50%",background:SYS_COLORS[sys],
                ...
                {cats.map(cat => {
                  const raw = s?.[cat];
                  const pa  = s?.PA || player.consensus?.PA || 1;
                  const val = per600 ? scaleTo600(raw, cat, pa) : raw;
                  return (
                    <td key={cat} style={{textAlign:"right",padding:"5px 5px",
                      color:val==null?"#2a2a3e":"#bbb",fontSize:11}}>
                      {fmt(val, cat)}
                    </td>
                  );
                })}
          ...
          <tr style={{borderTop:"2px solid #2a2a3e"}}>
            ...
            {cats.map(cat => {
              const raw = player.consensus?.[cat];
              ...
              return (
                <td key={cat} style={{textAlign:"right",padding:"5px 5px",fontWeight:700}}>
                  <span style={{color:"#4A9EFF",fontFamily:"'DM Mono',monospace",fontSize:11}}>
                    {fmt(val, cat)}
                  </span>
                  <DisBadge cv={cv}/>
                </td>
          ...
          <div key={cat} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{width:70,height:3,background:"#1a1a2e",borderRadius:2}}>
                <div style={{height:"100%",width:`${Math.min(cv*200,100)}%`,
...
function PlayerPanel({ player, allPlayers, per600, onClose, onNavigate }) {
  ...
  const PT = (t, label) => (
    <button key={t} onClick={()=>setPanelTab(t)}
      style={{padding:"5px 10px",fontSize:10,fontWeight:700,border:"none",cursor:"pointer",
        ...
        background:panelTab===t?"#1e1e2e":"transparent",
  ...
  return (
    <div style={{width:340,flexShrink:0,position:"sticky",top:16,
      ...
      background:"#0a0a12",border:"1px solid #1e1e2e",borderRadius:12,padding:18}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:14}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:11,color:"#444",marginTop:3,display:"flex",gap:6,flexWrap:"wrap"}}>
            ...
            <span>·</span>
            <span>{(player.positions||[player.pos]).join(", ")}</span>
            <span>·</span>
      ...
      {/* Panel tabs */}
      <div style={{display:"flex",gap:2,marginBottom:14,background:"#0f0f18",
      ...
      {panelTab === "overview" && (
        <div style={{background:"#0f0f18",border:"1px solid #1a1a2e",borderRadius:8,padding:14}}>
          ...
          {cats.map(cat => {
            ...
            return <PctBar key={cat} val={val} pct={pct} cat={cat}/>;
          })}
        </div>
      )}
              ...
              <table style={{width:"100%",borderCollapse:"collapse",
                ...
                <thead>
                  <tr>
                    ...
                    {Object.keys(player.history[0]).filter(k=>k!=="season").slice(0,8).map(k=>(
                ...
                <tbody>
                  {player.history.map(h=>(
                    <tr key={h.season} style={{borderTop:"1px solid #1a1a2e"}}>
                      <td style={{padding:"4px 6px",color:"#4A9EFF",fontWeight:700}}>{h.season}</td>
                      {Object.entries(h).filter(([k])=>k!=="season").slice(0,8).map(([k,v])=>(
...
function PlayerRow({ player, rank, isSelected, isDrafted, onSelect, onDraft, showDraftBtn, per600 }) {
  const tc = TIER_COLORS[player.tier] ?? "#6B7280";
  const cats = player.type === "hitter" ? ["HR","RBI","SB","OBP"] : ["W","K","ERA","WHIP"];
  const flags = player.flags ?? [];
  ...
  return (
    <div onClick={()=>onSelect(player)}
      ...
      <span style={{width:24,fontSize:10,color:"#2a2a3e",fontFamily:"'DM Mono',monospace",flexShrink:0}}>
        {rank}
      </span>

      <div style={{width:26,height:26,borderRadius:5,background:tc+"15",border:`1px solid ${tc}35`,
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          {hasFlag && <span style={{fontSize:9,color:"#F87171",flexShrink:0}}>⚠</span>}
        </div>
        <div style={{fontSize:10,color:"#333"}}>{player.team}</div>
      </div>
      ...
      <div style={{display:"flex",gap:12,fontSize:11,flexShrink:0}}>
        {cats.map(cat => {
          ...
          return (
            <div key={cat} style={{textAlign:"right",minWidth:30}}>
              <div style={{color:pctColor(ep),fontWeight:600,
                fontFamily:"'DM Mono',monospace",fontSize:11}}>{fmt(val,cat)}</div>
...
function ScarcityMeter({ scarcity, drafted, players }) {
  const positions = Object.keys(scarcity || {}).filter(p =>
    ["C","1B","2B","SS","3B","OF","SP","RP"].includes(p));
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080810",display:"flex",flexDirection:"column",
      ...
      <div style={{fontSize:12,color:"#333",marginTop:8}}>
        Make sure <code style={{color:"#4A9EFF",background:"#4A9EFF10",padding:"2px 6px",borderRadius:4}}>
          public/players.json</code> exists.<br/>
        Run <code style={{color:"#00C896",background:"#00C89610",padding:"2px 6px",borderRadius:4}}>
          python scripts/process.py</code> locally to generate it.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    return players
      .filter(p => typeFilter === "All" ||
        (typeFilter === "Hitters" && p.type === "hitter") ||
        (typeFilter === "Pitchers" && p.type === "pitcher"))
      .filter(p => posFilter === "All" ||
        (p.positions || [p.pos]).some(pos => pos === posFilter))
      .filter(p =>
  ...
  const Filters = () => (
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
      ...
      {(typeFilter === "Pitchers" ? POSITIONS_PIT :
        typeFilter === "Hitters"  ? POSITIONS_HIT : ["All"]).map(pos=>(
  ...
  const WithPanel = ({children}) => (
    <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
      ...
      {selected && (
        <PlayerPanel player={selected} allPlayers={filtered} per600={per600}
      ...
      <div style={{maxWidth:1500,margin:"0 auto",padding:"16px 24px"}}>
              ...
              {byTeam.map(([team,tp])=>(
                <div key={team} style={{marginBottom:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#555",textTransform:"uppercase",
                    letterSpacing:"0.8px",marginBottom:7,paddingBottom:5,
                    borderBottom:"1px solid #131320"}}>
                    {team} <span style={{color:"#2a2a3e",fontWeight:400,fontFamily:"'DM Mono',monospace"}}>
                      ({tp.length})
                    </span>
                  </div>
                  <PlayerList list={tp}/>
              ...
              {byPos.map(([pos,pp])=>(
                <div key={pos} style={{marginBottom:18}}>
                  <div style={{fontSize:11,fontWeight:700,color:"#555",textTransform:"uppercase",
                    letterSpacing:"0.8px",marginBottom:7,paddingBottom:5,
                    borderBottom:"1px solid #131320"}}>
                    {pos} <span style={{color:"#2a2a3e",fontWeight:400,fontFamily:"'DM Mono',monospace"}}>
                      ({pp.length})
                    </span>
                  </div>
                  <PlayerList list={pp}/>
        ...
        {tab === "draft" && (
          <>
            ...
            <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
              <div style={{flex:1,minWidth:0}}>
                <PlayerList list={filtered} showDraft={true}/>
              </div>
          ...
          <WithPanel>
            ...
            {filtered.map((p,i)=>(
              <PlayerRow key={p.id} player={p} rank={i+1}
                isSelected={selected?.id===p.id} isDrafted={drafted.has(p.id)}
                onSelect={handleSelect} onDraft={toggleDraft}
                showDraftBtn={false} per600={per600}/>
            ))}
          </WithPanel>
        )}
